...
class RegisterDialog(QtGui.QDialog):
    def __init__(self,parent=None):
        ...
        self.setup()
    def setup(self):
        QtCore.QObject.connect(self.ui.pushButton_reg, QtCore.SIGNAL(_fromUtf8("clicked()")), self.register)
    def register(self):
        srv_ip = self.ui.lineEdit_srv.text()
...
class T(QtCore.QObject, threading.Thread):
    ...
    def run(self):
        while self.runFlag:
            inputready,outputready,exceptready=select.select([self.sock],[],[self.sock],0.1)
            for s in inputready:
                data=s.recv(1024)
...
class MainWindow(QtGui.QMainWindow):
    ...
    def openChatDlg(self,item,col):
        if item in self.onlist.values() or item in self.offlist.values():
            ...
            chat_dlg = ChatDialog(self.user_no,self.nick, user_no, self.friends[user_no], self.srv_ip, parent=None)
    ...
    def passiveOpenChatDlg(self,user_no,msg):
        chat_dlg = ChatDialog(self.user_no,self.nick, user_no, self.friends[user_no], self.srv_ip, msg=msg, parent=None)
        self.runDlg[user_no]=chat_dlg
        chat_dlg.exec_()
        chat_dlg.destroy()
        self.runDlg.pop(user_no)
    ...
    def setupSignals(self):
        QtCore.QObject.connect(self.ui.pushButton_findfriend, QtCore.SIGNAL(_fromUtf8("clicked()")), self.find)
        QtCore.QObject.connect(self.ui.pushButton_addfriend, QtCore.SIGNAL(_fromUtf8("clicked()")), self.addFriend)
    def find(self):
    ...
    def setupTrayIcon(self):
        ...
        self.restoreAction = QtGui.QAction(u"显示窗口", self, triggered=self.showNormal)
        ...
        QtCore.QObject.connect(self.trayIcon, \
                               QtCore.SIGNAL("activated(QSystemTrayIcon::ActivationReason)"),self.sysTrayIcon_activated)
...
class ChatDialog(QtGui.QDialog):
    def __init__(self,user_no,nick,fno,fnick,srv_ip,msg="",parent=None):
        ...
        self.show()
    def setupSignals(self):
        QtCore.QObject.connect(self.ui.sendButton, QtCore.SIGNAL(_fromUtf8("clicked()")), self.sendMsg)
        QtCore.QObject.connect(self.ui.sendFileButton, QtCore.SIGNAL(_fromUtf8("clicked()")), self.sendFileThread)
        QtCore.QObject.connect(self.ui.recvFileButton, QtCore.SIGNAL(_fromUtf8("clicked()")), self.recvFileThread)
    def closeEvent(self,event):
        self.runFlag = 0
        if self.sock:
    ...
    def session(self,first_msg):
        ...
        if isinstance(first_msg,list):
            for i in range(0,len(first_msg),2):
                ...
                self.ui.textBrowser.append(u"<font color=red><b>[%s ] %s</b></font>" %(self.fnick,first_msg[i]))
                self.ui.textBrowser.append(first_msg[i+1].decode('utf-8'))
        elif len(first_msg) > 0:
            print u"%s from %s" %(first_msg,self.fnick)
            self.ui.textBrowser.append(u"<font color=red><b>[%s ] %s</b></font>" %(self.fnick,self.curTime()[1]))
        ...
        while self.runFlag:
            ...
            for s in writable:
                try:
                    ...
                    pass
    def curTime(self):
        return time.strftime('%Y-%m-%d %X', time.localtime(time.time())).split(' ')
    def sendMsg(self):
        msg = self.ui.textEdit.toPlainText()
        self.ui.textEdit.clear()
        self.ui.textBrowser.append(u"<font color=blue><b>[%s ] %s</b></font>" %(self.nick,self.curTime()[1]))
        self.ui.textBrowser.append(msg)
        self.sMsgQueue.put(msg)

    def sendFileThread(self):
        fileName = QtGui.QFileDialog.getOpenFileName(self, "Open text file", "Open new file", self.tr("All Files (*.*)"))
        if fileName.isEmpty():
            print u"没有选择文件"
            return
    ...
    def recvFileThread(self):
        ...
        for item in items:
            ...
            fileName = QtGui.QFileDialog.getSaveFileName(self, "Save As",initialPath,"Files (*.*);;All Files (*)")
            if fileName.isEmpty():
                #print "not select file"
                return
            t = threading.Thread(target=self.recvFile,args=(fileName, str(item.text())))
            t.start()
    def recvFile(self, savedFileName, itemName):
        net.recvFile(savedFileName, itemName, self.srv_ip)
        self.ui.textBrowser.append("<font color=blue>[%s ] %s</font>" %(self.nick,self.curTime()[1]))
        self.ui.textBrowser.append(u"%s接收成功，保存在%s" %(itemName,savedFileName))
#系统入口
